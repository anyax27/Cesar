...
function descifrar(texto, desplazamiento) {
    ...
    const letras = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
    //Si aceptamos desplazamientos negativos, necesitamos hacerlo dos veces. Si no, sería:
    //desplazamiento = desplazamiento % 26; (lo mismo xd)
    desplazamiento = (desplazamiento % 26 - 26) % 26; 
    return texto.replace(/[A-Z]/ig, c => letras[(letras.indexOf(c) - desplazamiento) % 26]);
}
...
function cifrar2(texto, desplazamiento) {
    ...
    if (texto){
        for (let i=0; i<texto.length; ++i){
            //Si la letra está en el array de letras (es un símbolo, un espacio, etc.)
            ...
            { 
                ...
                resultado+=texto[i]; // Números, espacios, símbolos, etc. 
        }
    }
    return resultado;
}